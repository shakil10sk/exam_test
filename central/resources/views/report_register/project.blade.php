...
@section('content')
    ...
    <div class="pd-x-20 pd-sm-x-30 pd-t-20 pd-sm-t-30">
        <h2 class="tx-gray-800 mg-b-5">প্রকল্পসমূহ</h2>
    </div>
        ...
        <div class="br-section-wrapper m-0 p-0">
            <div class="card ">
                <div class="card-header">
                    <form action="{{ route('report-register.project') }}" method="get">
                    ...
                    <table class="table datatable text-dark">
                        <thead>
                            <tr>
                                <th>নং</th>
                                <th>প্রকল্পের নাম</th>
                                <th>প্রকল্পের পূর্বের ছবি</th>
                                <th>ফাইল</th>
                                <th>প্রকল্পের চূড়ান্ত ছবি</th>
                                <th>ফাইল</th>
                                <th>বিস্তারিত</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($projects ?? [] as $key => $item)
                                <tr>
                                    <td>{{++$key}}</td>
                                    <td>{{$item->title}}</td>
                                    <td>{{$item->pre_photo}}</td>
                                    <td> <a href="{{env('ADMIN_URL').'\public\assets\reports\\'.$item->file}}" download="{{$item->title.$item->pre_photo}}">Download</a> </td>
                                    <td>{{$item->final_photo}}</td>
                                    <td> <a href="{{env('ADMIN_URL').'\public\assets\reports\\'.$item->file}}" download="{{$item->title.$item->final_photo}}">Download</a> </td>
                                    <td>{{$item->descrip}}</td>
                                </tr>
                            @endforeach
                        </tbody>

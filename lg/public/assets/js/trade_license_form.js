                ...
                success: function (res) {
                    ...
                    if (res.niderror) {
                        ...
                        Swal.fire({
                            ...
                            icon: 'success',
                            html: res.message + '</b> <a href="' + path + '/verify/' + res.application + '/' + res.tracking + '/' + res.unionid + '/' + res.type + '" type="button" class="btn btn-info" target="_blank">আবেদনটি প্রিন্ট করুন</a> ',
                            showConfirmButton: true,
                            showCancelButton: false,
                            focusConfirm: false,
                            confirmButtonText:
...
//get business type by there union id
$('#union-id').change(function(e){ 
    let web_loc = $('meta[name=url]').attr('content');

    $.ajax({
        type: 'post',
        url: web_loc + '/geo/code/business_type',
        data: { 'union_id': this.value },
        success: function (data) {
            ...
            data.forEach(item => {
                ...
                option += "<option value='" + item.id + "'>" + item.name_bn + "</option>";
            });

            $('#business_type').html(option);
        },

<!DOCTYPE HTML>
<html lang="en-US">
<head>
    <base href="">
    <meta charset="utf-8">
    <title>
        হোল্ডিং নামজারীর আবেদন পত্র
    </title>
    <style media="all" type="text/css">
        body {
            font-family: 'bangla', sans-serif !important;
            font-size: 14px;
        }

        @media print {
            * {
                -webkit-print-color-adjust: exact;
            }
        }

        @page {
            header: page-header;
            footer: page-footer;
            margin: 20px 0px;
            padding: 0px;
        }

        @media print {
            body {
                font-size: 14px !important;
                font-family: 'bangla', sans-serif !important;
            }
        }
    </style>
    </meta>
    </base>
</head>
<body>

@if(!empty($namjari))
    @if(! $print_setting->pad_print )
        @include('layouts.pdf_sub_layouts.application_header')

    @else
        <table>
            <tr>
                <td style="height: 150px"></td>
            </tr>
        </table>
    @endif
    <!-----------top area end--------------->
    <!-----------heading start--------------->
    <table border="0" cellpadding="5" cellspacing="5"
           style="border-collapse:collapse;margin-left: 50px; margin-top: 20px; " width="95%">
        <tr>
            <td style="text-align: center; {{($namjari->photo)?"padding-left:150px;":""}}">
                <h2>
                        <span style="border-bottom: 1px solid; ">
                            হোল্ডিং নামজারীর আবেদন পত্র
                        </span>
                </h2>
            </td>
            <td style="text-align: center; padding-bottom:50px;">

                @if($namjari->photo)
                    <img src="{{ asset('images/application/'. $namjari->photo) }}" height="80px" width="80px"/>
                @endif

            </td>
        </tr>
    </table>
    <!-----------heading end--------------->
    <table border="0" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin-left: 50px;" width="95%">
        <tr>
            <td colspan="1">
                আবেদনের তারিখ
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ BanglaConverter::bn_number(date('d-m-Y', strtotime($namjari->created_time))) }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px">
                পিন নং
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ BanglaConverter::bn_number($namjari->pin) }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;height: 20px">
                ট্রাকিং
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ BanglaConverter::bn_number($namjari->tracking) }}
                    </span>
            </td>
        </tr>
        <tr>

            <td colspan="4" style="text-align: center;">
                <table border="0" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin: 10px 0px;"
                       width="95%">
                    <tr>
                        <td>
                            <h5 style="margin-right: 50px; margin-bottom: 10px">
                        <span style=" border-bottom: 1px solid; ">
                            যে হোল্ডিং এর মালিকানা রেকর্ড সংশোধন করিতে হইবে তাহার
                        </span>
                            </h5>
                        </td>
                    </tr>
                </table>

            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                সাবেক মালিকের নাম
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->former_owner_bn }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                হোল্ডিং নাম্বার
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->holding_no }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1">
                পিতার নাম
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->former_owner_father_name_bn }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                মাতার নাম
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->former_owner_mother_name_bn }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1">
                বাৎসরিক মূল্যমান
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                          : {{ $namjari->yearly_rate }}
                    </span>
            </td>
            <td colspan="1" style="border-left: none">
                ত্রৈমাসিক পৌরকর
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->trimasik_tax }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1">
                সর্বশেষ এসেসমেন্ট সন
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                          : {{ $namjari->last_assesment_date }}
                    </span>
            </td>
        </tr>
        <tr>

            <td colspan="4" style="text-align: center;">
                <table border="0" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin: 10px 0px;"
                       width="95%">
                    <tr>
                        <td>
                            <h5 style="margin-right: 50px; margin-bottom: 10px">
                        <span style=" border-bottom: 1px solid; ">
                            সংশোধনক্রমে যে মালিকের নাম রেকর্ড করিতে হইবে তাহার
                        </span>
                            </h5>
                        </td>
                    </tr>
                </table>

            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                নাম
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->name_bn }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                পিতা নাম
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->father_name_bn }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                গ্রাম
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->permanent_village_bn }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                ওয়ার্ড নং
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->permanent_ward_no }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                উপজেলা
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->permanent_upazila_name }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                জেলা
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->permanent_district_name }}
                    </span>
            </td>
        </tr>
        <tr>

            <td colspan="4" style="text-align: center;">
                <table border="0" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin: 10px 0px;"
                       width="95%">
                    <tr>
                        <td>
                            <h5 style="margin-right: 50px; margin-bottom: 10px">
                        <span style=" border-bottom: 1px solid; ">
                            মালিকানা প্রাপ্তির পূর্ণ বিবরণী
                        </span>
                            </h5>
                        </td>
                    </tr>
                </table>

            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                প্রস্তাবিত ভূমির মৌজার নাম ও নম্বর
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->bhumi_moja_no }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                খতিয়ান নম্বর
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->khotian_no }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                দাগ নম্বর
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->dag_no }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                জমির পরিমাণ
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->land_amount }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                দলিল দাতার নাম
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->dolil_datar_name }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                রেজিঃকৃত দলিলের নম্বর
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->dolil_no }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                রেজিঃ অফিসের নাম
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->reg_office_name }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                রেজিঃ তারিখ
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->reg_date }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                দলিলের যে হোল্ডিং নম্বর ও মহল্লা উল্লেখ আছে
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->dolil_hold_no }}
                    </span>
            </td>
        </tr>
        <tr>

            <td colspan="4" style="text-align: center;">
                <table border="0" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin: 10px 0px;"
                       width="95%">
                    <tr>
                        <td>
                            <h5 style="margin-right: 50px; margin-bottom: 10px">
                        <span style=" border-bottom: 1px solid; ">
                            হোল্ডিং এর বিবরণী
                        </span>
                            </h5>
                        </td>
                    </tr>
                </table>

            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                বহুতলা দালান ও কোঠার সংখ্যা
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->bohuthola_dalan }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                একতলা দালান ও কোঠার সংখ্যা
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->ekthola_dalan }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                আধা পাকা ঘর ও কোঠার সংখ্যা
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->ada_faka_ghor }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                কাঁচা ঘরের সংখ্যা
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->kaca_ghor }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                পায়খানার সংখ্যা
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->latrin_no }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                জলের টেপের সংখ্যা
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->jhol_tap_no }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                নলকূপের সংখ্যা
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->tubewel_no }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                দোকান ও কারখানার সংখ্যা
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->dokan_no }}
                    </span>
            </td>
        </tr>
        <tr>

            <td colspan="4" style="text-align: center;">
                <table border="0" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin: 10px 0px;"
                       width="95%">
                    <tr>
                        <td>
                            <h5 style="margin-right: 50px; margin-bottom: 10px">
                        <span style=" border-bottom: 1px solid; ">
                           হোল্ডিং এ বসবাসকারী পরিবারের সংখ্যা
                        </span>
                            </h5>
                        </td>
                    </tr>
                </table>

            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                কতটি পরিবার বাস করে
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->family_no }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                কি শর্তে বাস করে
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->conditions }}
                    </span>
            </td>
        </tr>
        <tr>
            <td colspan="1" style="height: 20px;">
                মোট মাসিক ভাড়া
            </td>
            <td colspan="2" style="border-left:none;border-bottom:none;">
                    <span>
                        : {{ $namjari->monthly_rant_rate }}
                    </span>
            </td>
            <td colspan="1" style="border-left:none;border-bottom:none;">
                ভাড়া আদায়ের শেষ তারিখ
            </td>
            <td colspan="2" style="border-left:none;">
                    <span>
                        : {{ $namjari->rant_last_date }}
                    </span>
            </td>
        </tr>
        <tr>
           <td colspan="4" >
               হলফ/শপথঃ আমি আবেদনকারী এই মর্মে শপথ করেতেছি যে, উপরোক্ত বিবরণীতে কোন সত্য গোপন করি নাই।যদি করিয়া থাকি তবে নামজারী/পৃথকীকরণের প্রস্তাব বাতিল বলিয়া গণ্য হইবে।
           </td>
        </tr>
    </table>
    <!-----------application area end--------------->
    {{-- verification area start --}}
    

			<table width="95%"  cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;margin-left: 50px; margin-top: 20px; ">
            	<tr>
            		<td style="text-align: center; font-size: 20px;font-weight: bold;">
            			<span style="border-bottom: 1px solid">সত্যায়ন / Verification</span>
            		</td>
            	</tr>
            </table>
                <br/>
                <hr/>
			<table width="95%"  cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;margin-left: 50px; margin-top: 20px; ">
                    <tr>
						<td  style="text-align: center;">
							<span style="border-top: 1px solid;" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;কাউন্সিলর&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							</span>
							  <br/>
						</td>  
                        @if ($print_setting->member)
                        <td style="text-align: center;">
                            <span style="border-top: 1px solid" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;সচিব&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            </span>	
                            <br/>					
                        </td>  
                        @endif
                        @if ($print_setting->chairman)
                        <td style="text-align: center;">
                            <span style="border-top: 1px solid" >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; মেয়র &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                </span>		
                                 <br/> 					
                        </td>                   
                        @endif
				    </tr>  
                    <tr style="padding-top:50px;">
                        <td valign="bottom" style="text-align: center; border-bottom: 1px dotted;font-size:13px;height:50px;">
                            <h2>নির্দেশনাবলী / Instruction</h2>
                        </td>
                    </tr>
                    <tr>
                        <td colspan="{{$colspan+1}}" style="border-bottom:1px solid black;font-size:13px">
                        <span>
                    ১) আবেদন পত্রটি  সংশ্লিষ্ট সাধারণ ওয়ার্ড কাউন্সিলর এবং সংরক্ষিত আসনে কাউন্সিলর কর্তৃক স্বাক্ষর নিয়ে নির্ধারিত শাখায় জমা দিন। 
                </span>
                <br>
                <span>
                    ২) পৌর কর পরিশোধ এর ভাউচার সাথে জমা দিন। 
                </span>
                <br>
                <span>
                    ৩) জন্ম নিবন্ধন / জাতীয় পরিচয় পত্রের ফটোকপি সাথে জমা দিন।   
                </span>
                <br>
                    <span>
                    ৪) সদ্য তোলা পাসপোর্ট সাইজের ছবি ১ কপি।
                </span>
                <br>
                    <span>
                    ৫) আবেদন কপি টি প্রিন্ট করে ১৫দিনের মধ্যে সংশ্লিষ্ট শাখায় জমা দিন। অন্যথায় আবেদন টি বাতিল বলে গণ্য হবে।
                </span>
                        </td>
                        <td rowspan="4" style="height:140px;width:160px; border-top:1px solid black; border-left:1px solid black;">
                            <?php

                            $url = $url . '/verify/prottyon_application/' . $namjari->tracking . '/' . $namjari->union_id . '/' . $namjari->type;

                            ?>
                            <img height="130"
                                src="data:image/png;base64, {!! base64_encode(QrCode::format('png')->size(200)->generate($url)) !!} "
                                width="170">
                        </td>
                    </tr>				
			</table>


    {{-- verification area end --}}
    <!------------------ footer area start-------------------->
    <table border="0" cellpadding="0" cellspacing="0"
           style="border-collapse:collapse;margin-left: 50px; margin-top: 20px;" width="95%">
        <tr>
            <td>
                <p>

                    E-mail : {{ $union->email }}
                </p>
            </td>
            <td style="text-align: right;">
                <p>
                    Developed by: Innovation IT
                    <br/>
                    www.innovationit.com.bd
                </p>
            </td>
        </tr>
    </table>
    <!------------------ footer area end-------------------->

@else
    <h2 style="text-align: center;color:red;">দুঃখিত ! আবেদনটি পাওয়া যায়নি</h2>
@endif
</body>
</html>

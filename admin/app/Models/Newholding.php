...
class Newholding extends Model
{
    use SoftDeletes;

    //==== new holding application store====//
    public function data_store($request)
    {
        ...
        // newholding optional info
        $newholding_option_info = [
            'union_id' => $request->union_id,
            'pin' => $request->pin,
            'tracking' => $request->tracking,
            'nearby_holding_no' => $request->nearby_holding_no,
            'holding_construction_date' => $request->holding_construction_date,
            'description_of_house' => $request->description_of_house,
        ...
        // submitted_papers //
        if ($request->hasFile("submitted_papers")) {
            //insert image
            $submitted_papers = $request->file("submitted_papers");

            $submitted_papers_img = $request->pin.substr(mt_rand(999999,999999999),0,5)."." .$submitted_papers->getClientOriginalExtension();

            $location = public_path("assets/images/application/" .$submitted_papers_img);

            //upload image in folder
            $move = Image::make($submitted_papers)->resize(300, 300)->save($location);

            if ($move) {
                $newholding_option_info['submitted_papers'] = $submitted_papers_img;
            }
        }

        // loan_papers //
        if ($request->hasFile("loan_papers")) {
            //insert image
            $loan_papers = $request->file("loan_papers");
            $loan_papers_img = $request->pin.substr(mt_rand(999999,100000000),-5)."." .
                $loan_papers->getClientOriginalExtension();

            $location = public_path("assets/images/application/" . $loan_papers_img);

            //upload image in folder
            $move = Image::make($loan_papers)->resize(300, 300)->save($location);

            if ($move) {
                $newholding_option_info['loan_papers'] = $loan_papers_img;
            }
        }

        //db transection start
        DB::transaction(function () use ($request, $citizen_data, $application_data,$newholding_option_info) {
            ...
            // newholding table data insert
            $newholding_insert = DB::table("newholding_optional_info")->insert($newholding_option_info);
        });
    ...
    //new holding preview data
    public function newholding_information($tracking = null, $union_id = null, $type = null)
    {
    ...
    //===death edit data=======//
    public function newholding_data($tracking, $union_id = null)
    {
        ...
        $data = DB::table('application AS APP')
            ...
            ->select('APP.*', 'APP.id as application_id', 'CTZ.*', 'CTZ.id as citizen_id', 'BDL1.bn_name as permanent_district_name_bn', 'BDL1.en_name as permanent_district_name_en', 'BDL2.bn_name as permanent_upazila_name_bn', 'BDL2.en_name as permanent_upazila_name_en', 'BDL3.bn_name as permanent_postoffice_name_bn', 'BDL3.en_name as permanent_postoffice_name_en', 'BDL4.bn_name as present_district_name_bn', 'BDL4.en_name as present_district_name_en', 'BDL5.bn_name as present_upazila_name_bn', 'BDL5.en_name as present_upazila_name_en', 'BDL6.bn_name as present_postoffice_name_bn', 'BDL6.en_name as present_postoffice_name_en','NHOI.*','NHOI.id as newholding_id')
    ...
    public function death_certificate_data($sonod_no = null, $union_id = null, $type = null)
    {
    ...
    public function update_newholding($receive)
    {
        ...
        // newholding optional info
        $newholding_option_info = [
            'nearby_holding_no' => $receive->nearby_holding_no,
            'holding_construction_date' => $receive->holding_construction_date,
            'description_of_house' => $receive->description_of_house,
            'holding' => $receive->holding,
        ...
        // submitted_papers //
        if ($receive->hasFile("submitted_papers")) {
            //insert image
            $submitted_papers = $receive->file("submitted_papers");

            $submitted_papers_img = $receive->pin.substr(mt_rand(999999,999999999),0,5)."." .$submitted_papers->getClientOriginalExtension();
        ...
        // loan_papers //
        if ($receive->hasFile("loan_papers")) {
            //insert image
            $loan_papers = $receive->file("loan_papers");
            $loan_papers_img = $receive->pin.substr(mt_rand(999999,100000000),-5)."." .
                $loan_papers->getClientOriginalExtension();
        ...
        DB::transaction(function () use ($receive, $citizen_update_data, $application_update_data,$newholding_option_info) {
            ...
            $newholding_update = DB::table("newholding_optional_info")
                ->where('id', $receive->newholding_id)
                ->update($newholding_option_info);
        });
    ...
    //===death info delete===//
    public function newholding_info_delete($request)
    {
        ...
        $newholding_delete = DB::table('newholding_optional_info')
            ->where('id', $request->newholdingId)
            ->update(['deleted_at' => Carbon::now(), 'is_active' => 0, 'updated_by' => Auth::user()->employee_id, 'updated_time' => Carbon::now(), 'updated_by_ip' => $request->ip()]);
    ...
    //===death applicant list data=====//

    public function newholding_applicant_list_data($receive, $search_content)
    {
        DB::enableQueryLog();

        $query = DB::table('application AS APP')

            ->select(DB::raw('SQL_CALC_FOUND_ROWS APP.id as application_id'), 'APP.type', 'APP.pin', 'APP.tracking', 'CTZ.id as citizen_id', 'CTZ.name_bn', 'CTZ.photo', 'CTZ.father_name_bn', 'CTZ.mobile', 'CTZ.union_id', 'CTZ.permanent_district_id', 'CTZ.permanent_upazila_id', 'CTZ.permanent_postoffice_id', 'APP.created_time','NHOI.id as newholding_id')
            ...
            ->join('newholding_optional_info AS NHOI', function ($join) {
                $join->on("NHOI.tracking", "=", "APP.tracking")
                    ->on("NHOI.pin", "=", "APP.pin")
                    ->on("NHOI.union_id", "=", "APP.union_id")
                    ->where('NHOI.is_active',1);
    ...
    //====death certificate list data====//
    public function death_certificate_list($receive, $search_content)
    {
        ...
        $query = DB::table('certificate AS CRT')
            ...
            ->where([
                ...
                ['CTZ.is_active', '=', 1],
                ['CRT.is_active', '=', 1],
                ['CRT.fiscal_year_id', '=', Global_model::current_fiscal_year()],
            ])
    ...
    //get death register data

    public function death_register_data($union_id = null, $type = null, $from_date = null, $to_date = null)
    {

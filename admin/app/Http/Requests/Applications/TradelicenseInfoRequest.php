...
class TradelicenseInfoRequest extends FormRequest
{
    /**
    ...
    {
        $validation = [
            'name_Of_organization_en' => ['string', 'max:255', 'nullable'],
            'name_Of_organization_bn' => ['required', 'string', 'max:255'],
            ...
            'nid.*' => ['numeric', 'max:99999999999999999', Rule::unique('citizen_information', 'nid')->where(function ($query) {
       ...
       if ( (int) $r->type_of_organization != 4){
           $validation += [
               'birth_id.*' => ['numeric', 'max:99999999999999999', Rule::unique('citizen_information', 'birth_id')->where(function ($query) {
                   $query->whereNull('deleted_at');
               }), 'nullable'],
        ...
        if ($r->pin) {
            unset($validation['nid.*'], $validation['birth_id.*']);
        }

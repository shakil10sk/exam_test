...
use App\Http\Requests\Applications\NewholdingInfoRequest;
use App\Http\Requests\Applications\NewholdingUpdateInfoRequest;
use App\Http\Requests\Applications\WebFormRequestRule;
use App\Http\Requests\Applications\WebNewholdingInfoRequest;
use App\Models\Death;
use App\Models\Global_model;
use App\Models\IdGenerate;
use App\Models\Newholding;
...
class NewholdingController extends Controller
{
    public $type;
    ...
    {
        return view('newholding.application');
    }
    ...
    public function store(NewholdingInfoRequest $request)
    {
        ...
        if ($response['status'] == "success") {
            if (isset($request->web)) {
                ...
                return response()->json(['success' => 'আবেদনটি সম্পূর্ণ হয়েছে!', 'tracking' => $tracking, 'pin' =>
                    $pin, 'unionid' => $union_id, 'type' => $this->type, 'application' => 'newholding_application']);
            } else {
                Alert::alert()->html("<i>আবেদনটি সম্পূর্ণ হয়েছে!</i>", "<p>" . $response['message'] . "</p><a href='" . route('newholding_preview', ['tracking' => $tracking]) . "' class='btn btn-info' target='_blank'><i class='icon-copy fa fa-eye' aria-hidden='true'></i> প্রিন্ট করুন</a><a href='applicant_list' class='btn btn-info ml-2'><i class='icon-copy fa fa-print' aria-hidden='true'></i>আবেদন তালিকা</a>", 'success')->showConfirmButton($btnText = 'ঠিক আছে', $btnColor = '#3085d6')->persistent(false, true);
    ...
    public function webApplly(WebNewholdingInfoRequest $request)
    {
        ...
        $res = Newholding::webApplication($request);

        if ($res) {
            if (isset($request->web)) {
                return response()->json(['success' => 'আবেদনটি সম্পূর্ণ হয়েছে!', 'tracking' => $tracking, 'pin' => $request->pin, 'unionid' => $union_id, 'type' => $this->type, 'application' => 'newholding_application']);
            } else {
                Alert::alert()->html("<i>আবেদনটি সম্পূর্ণ হয়েছে!</i>", "<p>আপনার পিন নং " . $request->pin . " এবং ট্র্যাকিং নং " . $request->tracking . "</p><a href='" . route('newholding_preview', ['tracking' => $tracking]) . "' class='btn btn-info' target='_blank'><i class='icon-copy fa fa-eye' aria-hidden='true'></i> প্রিন্ট করুন</a><a href='applicant_list' class='btn btn-info ml-2'><i class='icon-copy fa fa-print' aria-hidden='true'></i>আবেদন তালিকা</a>", 'success')->showConfirmButton($btnText = 'ঠিক আছে', $btnColor = '#3085d6')->persistent(false, true);
                return redirect()->back();
            }
        } else {
            if (isset($request->web)) {
    ...
    //===newholding application preview===//
    public function preview($tracking = null)
    {
        $preview = new Newholding();

        //get union id
        $union_id = Auth::user()->union_id;

        $response = $preview->newholding_information($tracking, $union_id, $this->type);
        ...
        $data = ['newholding' => $response, 'union' => $union_profile];
        ...
        $pdf = PDF::loadView('newholding.ppreview', $data);

        return $pdf->stream('newholding_preview.pdf');
    }
    ...
    public function applicant_list()
    {
        ...
        return view('newholding.applicant_list', compact("fiscal_year_list"));
    }
    ...
    public function applicant_data(Request $request)
    {
        ...
        $applicant_list = new Newholding();

        $response = $applicant_list->newholding_applicant_list_data($request_data, $search_content);
    ...
    public function sonod_print_bn($sonod_no = null)
    {
        ...
        //get death certificate data
        $response = $death->death_certificate_data($sonod_no, $union_id, $this->type);
        ...
        if (!empty($response)) {
            ...
            $data = ['death' => $response, 'union' => $union_profile];
            ...
            // $pdf = PDF::loadView('death.bangla_certificate', $data);
            ...
            $pdf = PDF::loadHtml(view('death.bangla_certificate', $data), $config);

            return $pdf->stream('death_certificate.pdf');
        } else {
    ...
    public function sonod_print_en($sonod_no = null)
    {
        ...
        if (!empty($response)) {
            ...
            //certificate pdf convert
            // $pdf = PDF::loadView('death.english_certificate', $data);
            $config = ['instanceConfigurator' => function ($mpdf) use ($union_profile) {
                $mpdf->SetWatermarkImage(asset('images/union_profile/' . $union_profile->main_logo));
                $mpdf->showWatermarkImage = true;
            }];

            $pdf = PDF::loadHtml(view('death.english_certificate', $data), $config);
    ...
    public function edit($tracking)
    {
        ...
        $response = $preview->newholding_data($tracking, $union_id);

        //   dd($response);


        return view('newholding.edit')->with('newholding', $response);
    }

    public function update(NewholdingUpdateInfoRequest $request)
    {
        $newholding = new Newholding();



        $response = $newholding->update_newholding($request);
    ...
    {
        $newholding = new Newholding();
        $response = $newholding->newholding_info_delete($request);
        return $response;
    }
    ...
    public function certificate_list()
    {
        ...
        return view('death.certificate_list');
    }

    //======for death certificate list data====//
    public function certificate_list_data(Request $request)
    {
        ...
        $response = $death->death_certificate_list($request_data, $search_content);
    ...
    public function money_receipt($sonod_no = null)
    {
        ...
        $response = $death->money_receipt_data($sonod_no, $union_id, $this->type);

        $bank = DB::table('bank')->where('sonod_type','=',92)->first();
        ...
        if (!empty($response)) {
            ...
            $pdf = PDF::loadView('death.money_receipt', ['data' => $response,'bank' => $bank, 'union' => $union_profile]);

            return $pdf->stream('death_money_receipt.pdf');
        } else {
    ...
    public function register($from_date = null, $to_date = null)
    {
        ...
        $response = $death->death_register_data($union_id, $this->type, $from_date, $to_date);
        ...
        $pdf = PDF::loadHtml(view('death.register',  ['data' => $response, 'union' => $union_profile]), $config);
        // $pdf = PDF::loadView('death.register',  ['data' => $response, 'union' => $union_profile]);

        return $pdf->stream("death_register.pdf");
    }

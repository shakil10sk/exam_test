...
use App\Http\Requests\Applications\HoldingNamjariInfoRequest;
use App\Http\Requests\Applications\HoldingNamjariUpdateRequest;
use App\Http\Requests\Applications\WebFormRequestRule;
use App\Http\Requests\Applications\WebHoldingNamjariInfoRequest;
use App\Models\Global_model;
use App\Models\HoldingNamjari;
...
class HoldingNamjariController extends Controller
{
    public $type;
    ...
    {
        return view('namjari.application');
    }
    ...
    public function store(HoldingNamjariInfoRequest $request)
    {
        ...
        if ($response['status'] == "success") {
            if (isset($request->web)) {
                ...
                return response()->json(['success' => 'আবেদনটি সম্পূর্ণ হয়েছে!', 'tracking' => $tracking, 'pin' => $pin, 'unionid' => $union_id, 'type' => $this->type, 'application' => 'namjari_application']);

            } else {
                Alert::alert()->html("<i>আবেদনটি সম্পূর্ণ হয়েছে!</i>", "<p>" . $response['message'] . "</p><a href='" . route('holdingnamjari_preview', ['tracking' => $tracking]) . "' class='btn btn-info' target='_blank'><i class='icon-copy fa fa-eye' aria-hidden='true'></i> প্রিন্ট করুন</a><a href='applicant_list' class='btn btn-info ml-2'><i class='icon-copy fa fa-print' aria-hidden='true'></i>আবেদন তালিকা</a>", 'success')->showConfirmButton($btnText = 'ঠিক আছে', $btnColor = '#3085d6')->persistent(false, true);
    ...
    public function webApplly(WebHoldingNamjariInfoRequest $request)
    {
        ...
        $res = HoldingNamjari::webApplication($request);

        if ($res) {
            if (isset($request->web)) {
                return response()->json(['success' => 'আবেদনটি সম্পূর্ণ হয়েছে!', 'tracking' => $tracking, 'pin' => $request->pin, 'unionid' => $union_id, 'type' => $this->type, 'application' => 'holdingnamjari']);
            } else {
                Alert::alert()->html("<i>আবেদনটি সম্পূর্ণ হয়েছে!</i>", "<p>আপনার পিন নং " . $request->pin . " এবং ট্র্যাকিং নং " . $request->tracking . "</p><a href='" . route('holdingnamjari_preview', ['tracking' => $tracking]) . "' class='btn btn-info' target='_blank'><i class='icon-copy fa fa-eye' aria-hidden='true'></i> প্রিন্ট করুন</a><a href='applicant_list' class='btn btn-info ml-2'><i class='icon-copy fa fa-print' aria-hidden='true'></i>আবেদন তালিকা</a>", 'success')->showConfirmButton($btnText = 'ঠিক আছে', $btnColor = '#3085d6')->persistent(false, true);
                return redirect()->back();
            }
        } else {
            if (isset($request->web)) {
    ...
    //===namjari application preview===//
    public function preview($tracking = null)
    {
        $preview = new HoldingNamjari();

        //get union id
        $union_id = Auth::user()->union_id;

        $response = $preview->namjari_information($tracking, $union_id, $this->type);

        //        dd($response);

        $union_profile = Global_model::union_profile($union_id);

        $data = ['namjari' => $response, 'union' => $union_profile];
        ...
        $pdf = PDF::loadView('namjari.ppreview', $data);

        return $pdf->stream('namjari_preview.pdf');
    }
    ...
    public function applicant_list()
    {
        ...
        return view('namjari.applicant_list', compact("fiscal_year_list"));
    }
    ...
    public function applicant_data(Request $request)
    {
        ...
        $applicant_list = new HoldingNamjari();

        $response = $applicant_list->namjari_applicant_list_data($request_data, $search_content);
    ...
    public function sonod_generate(Request $request)
    {
        ...
        $namjari = new HoldingNamjari();



        $response = $namjari->sonod_generate($request);
    ...
    public function sonod_print_bn($sonod_no = null)
    {
        ...
        //get namjari certificate data
        $response = $namjari->namjari_certificate_data($sonod_no, $union_id, $this->type);
        ...
        if (!empty($response)) {
            ...
            $data = ['namjari' => $response['certificate_data'], 'fee_data'=> $response['fee_data'], 'union' => $union_profile];
            ...
            // $pdf = PDF::loadView('namjari.bangla_certificate', $data );
            ...
            $pdf = PDF::loadHtml(view('namjari.bangla_certificate', $data), $config);

            return $pdf->stream('namjari_certificate.pdf');
    ...
    public function note($sonod_no = null)
    {
        ...
        if (!empty($response)) {
            ...
            $data['colspan'] = $data['print_setting']->chairman + $data['print_setting']->member + $data['print_setting']->sochib + $data['print_setting']->obibabok;
            //certificate pdf convert
            // $pdf = PDF::loadView('namjari.english_certificate', $data);
            $config = ['instanceConfigurator' => function ($mpdf) use ($union_profile) {
                $mpdf->SetWatermarkImage(asset('images/union_profile/' . $union_profile->main_logo));
                $mpdf->showWatermarkImage = true;
            }];

            $pdf = PDF::loadHtml(view('namjari.note', $data), $config);

            return $pdf->stream('namjari_note.pdf');
    ...
    public function edit($tracking)
    {
        ...
        $response = $preview->namjari_data($tracking, $union_id);

        return view('namjari.edit')->with('namjari', $response);
    }

    public function update(HoldingNamjariUpdateRequest $request)
    {
        $namjari = new HoldingNamjari();

        $response = $namjari->update_namjari($request);
    ...
    {
        $namjari = new HoldingNamjari();
        $response = $namjari->namjari_info_delete($request);

        return $response;
    }

    //=====for namjari certificate list====//
    public function certificate_list()
    {
        ...
        return view('namjari.certificate_list', compact("fiscal_year_list"));
    }

    //======for namjari certificate list data====//
    public function certificate_list_data(Request $request)
    {
        ...
        $response = $namjari->namjari_certificate_list($request_data, $search_content);
    ...
    public function money_receipt($sonod_no = null)
    {
        ...
        $response = $namjari->money_receipt_data($sonod_no, $union_id, $this->type);

        //        dd($response);
        $bank = DB::table('bank')->where('sonod_type','=',93)->first();
        ...
        if (!empty($response)) {
            ...
            $pdf = PDF::loadView('namjari.money_receipt', ['data' => $response, 'bank' => $bank,'union' => $union_profile]);

            return $pdf->stream('namjari_money_receipt.pdf');
    ...
    public function register($from_date = null, $to_date = null)
    {
        ...
        $response = $namjari->namjari_register_data($union_id, $this->type, $from_date, $to_date);
        ...
        $pdf = PDF::loadHtml(view('namjari.register', ['data' => $response, 'union' => $union_profile]), $config);
        // $pdf = PDF::loadView('namjari.register',  ['data' => $response, 'union' => $union_profile]);

        return $pdf->stream("namjari_register.pdf");

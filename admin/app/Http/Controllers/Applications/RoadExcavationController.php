...
use App\Http\Requests\Applications\RoadExcavationRequest;
use App\Http\Controllers\Controller;
use App\Http\Requests\Applications\WebRoadExcavationInfoRequest;
use Illuminate\Support\Facades\Auth;
use RealRashid\SweetAlert\Facades\Alert;
use Illuminate\Http\Request;
use App\Models\RoadExcavation;
...
class RoadExcavationController extends Controller
{
    /**
    ...
    {
        return view('road_excavation.application');
    }
    ...
    public function store(RoadExcavationRequest $request)
    {
        // dd($request);
        ...
        {
            if(isset($request->web)){
                return response()->json(['success'=> 'আবেদনটি সম্পূর্ণ হয়েছে!', 'tracking' => $tracking, 'pin' => $pin, 'unionid' => $union_id, 'type' => $this->type, 'application' => 'road_excavation_application']);

            } else {
                Alert::alert()->html("<i>আবেদনটি সম্পূর্ণ হয়েছে!</i>","<p>".$response['message']."</p><a href='".route('road_preview', ['tracking' => $tracking])."' class='btn btn-info' target='_blank'><i class='icon-copy fa fa-eye' aria-hidden='true'></i> প্রিন্ট করুন</a><a href='applicant_list' class='btn btn-info ml-2'><i class='icon-copy fa fa-print' aria-hidden='true'></i>আবেদন তালিকা</a>",'success')->showConfirmButton($btnText = 'ঠিক আছে', $btnColor = '#3085d6')->persistent(false,true);
    ...
    //=== Road Excavation application preview ===//
    public function preview( $tracking = null)
    {
        $preview = new RoadExcavation();

        //get union id
        $union_id = Auth::user()->union_id;

        $response = $preview->preview_data($tracking, $union_id, $this->type);
        ...
        $pdf = PDF::loadView('road_excavation.ppreview', $data);

        return $pdf->stream('road_excavation_preview.pdf');
    }
    ...
    public function applicant_list()
    {
        ...
        return view('road_excavation.applicant_list', compact("fiscal_year_list"));
    }
    ...
    public function applicant_data(Request $request)
    {
        ...
        $applicant_list = new RoadExcavation();

        $response = $applicant_list->applicant_list_data($request_data, $search_content);
    ...
    public function sonod_generate(Request $request)
    {
        ...
        $response = $data->sonod_generate($request);

        echo json_encode($response);
    }

    // === road excavation bangla sonod print === //
    public function sonod_print_bn($sonod_no = null)
    {
       ...
       // get road certificate data
        $response = $data->certificate_data($sonod_no, $union_id, $this->type);
        ...
        if (!empty($response)) {
            ...
            $pdf = PDF::loadHtml(view('road_excavation.bangla_certificate', $data), $config);

            return $pdf->stream('road_excavation_certificate.pdf');
    ...
    //=== Road Excavation english sonod print ====//
    public function sonod_print_en($sonod_no = null)
    {
       ...
       // get road excavation certificate data
        $response = $data->certificate_data($sonod_no, $union_id, $this->type);
        ...
        if (!empty($response)) {
            ...
            $pdf = PDF::loadHtml(view('road_excavation.english_certificate', $data), $config);
    ...
    public function edit($tracking)
    {
        ...
        $response = $preview->edit_data($tracking, $union_id);

        // dd($response);

        return view('road_excavation.edit')->with('data', $response);
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function update(RoadExcavationRequest $request)
    {
        $data = new RoadExcavation();

        $response = $data->update_data($request);
    ...
    {
        $data = new RoadExcavation();
        $response = $data->delete_data($request);

        return $response;
    }

    //==== road excavation certificate list ====//
    public function certificate_list()
    {
        ...
        return view('road_excavation.certificate_list', compact("fiscal_year_list"));
    }
    ...
    public function certificate_list_data(Request $request)
    {
        ...
        $response = $data->certificate_list($request_data, $search_content);
    ...
    // Road Excavation money receipt
    public function money_receipt($sonod_no = null, $voucher_no = null)
    {
        $data = new RoadExcavation();
        ...
        $response = $data->money_receipt_data($sonod_no, $voucher_no, $union_id, $this->type);

        $bank = DB::table('bank')->where('sonod_type','=',4)->first();
        ...
        $bank = DB::table('bank')->where('sonod_type','=',94)->first();
        ...
        if (!empty($response)) {
            $pdf = PDF::loadView('road_excavation.money_receipt', ['data' => $response, 'bank' => $bank, 'union' => $union_profile]);

            return $pdf->stream('road_excavation_money_receipt.pdf');
        }else{
    ...
    public function webApplly(WebRoadExcavationInfoRequest $request)
    {
        ...
        $res = RoadExcavation::webApplication($request);

        if($res){
            if(isset($request->web)){
                return response()->json(['success'=> 'আবেদনটি সম্পূর্ণ হয়েছে!', 'tracking' => $tracking, 'pin' => $request->pin, 'unionid' => $union_id, 'type' => $this->type, 'application' => 'voter_application']);
            }else {
                Alert::alert()->html("<i>আবেদনটি সম্পূর্ণ হয়েছে!</i>","<p>আপনার পিন নং ".$request->pin." এবং ট্র্যাকিং নং ".$request->tracking."</p><a href='".route('voter_preview', ['tracking' => $tracking])."' class='btn btn-info' target='_blank'><i class='icon-copy fa fa-eye' aria-hidden='true'></i> প্রিন্ট করুন</a><a href='applicant_list' class='btn btn-info ml-2'><i class='icon-copy fa fa-print' aria-hidden='true'></i>আবেদন তালিকা</a>",'success')->showConfirmButton($btnText = 'ঠিক আছে', $btnColor = '#3085d6')->persistent(false,true);
                return redirect()->back();
            }
        }else {
            if(isset($request->web)){
    ...
    public function register($from_date = null, $to_date = null){
        ...
        // get register data
        $response = $data->register_data($union_id, $this->type, $from_date, $to_date);
        ...
        $pdf = PDF::loadHtml(view('road_excavation.register',  ['data' => $response, 'union' => $union_profile]), $config);

        return $pdf->stream("road_excavation_register.pdf");
    }

    public function register_show(){
        return view('road_excavation.register_list');
    }
}

...
use App\Http\Requests\Applications\LandUseRequest;
use App\Http\Controllers\Controller;
use App\Http\Requests\Applications\WebLandUseInfoRequest;
...
class LandUseController extends Controller
{
    ...
    {
        return view('land_use.application');
    }
    ...
    public function store(LandUseRequest $request)
    {
        ...
        {
            if(isset($request->web)){
                return response()->json(['success'=> 'আবেদনটি সম্পূর্ণ হয়েছে!', 'tracking' => $tracking, 'pin' => $pin, 'unionid' => $union_id, 'type' => $this->type, 'application' => 'land_use_application']);

            } else {
                Alert::alert()->html("<i>আবেদনটি সম্পূর্ণ হয়েছে!</i>","<p>".$response['message']."</p><a href='".route('land_use_preview', ['tracking' => $tracking])."' class='btn btn-info' target='_blank'><i class='icon-copy fa fa-eye' aria-hidden='true'></i> প্রিন্ট করুন</a><a href='applicant_list' class='btn btn-info ml-2'><i class='icon-copy fa fa-print' aria-hidden='true'></i>আবেদন তালিকা</a>",'success')->showConfirmButton($btnText = 'ঠিক আছে', $btnColor = '#3085d6')->persistent(false,true);
    ...
    public function preview( $tracking = null)
    {
        ...
        //  return view('land_use.ppreview', $data);

        $pdf = PDF::loadView('land_use.ppreview', $data);

        return $pdf->stream('land_use_preview.pdf');
    }
    ...
    public function applicant_list()
    {
        ...
        return view('land_use.applicant_list', compact("fiscal_year_list"));
    }
    ...
    public function sonod_regenerate(Request $request)
    {
        ...
        $response = $land_use->sonod_generate($request);

        echo json_encode($response);
    }

    // === land use  bangla sonod print === //
    public function sonod_print_bn($sonod_no = null)
    {
       ...
       // get road certificate data
        ...
        if (!empty($response)) {
            $data = ['certificate_data' => $response['certificate_data'], 'union' => $union_profile];
            ...
            $pdf = PDF::loadHtml(view('land_use.bangla_certificate', $data), $config);

            return $pdf->stream('land_use_certificate.pdf');
    ...
    public function note($sonod_no = null)
    {
        ...
        if (!empty($response)) {
            $data = ['certificate_data' => $response['certificate_data'], 'fee_data' =>$response['fee_data'],
                'union' => $union_profile];
            ...
            $pdf = PDF::loadHtml(view('land_use.note', $data), $config);

            return $pdf->stream('land_use_note.pdf');
    ...
    public function edit($tracking)
    {
        ...
        return view('land_use.edit')->with('data', $response);
    }

    public function update(LandUseRequest $request)
    {
    ...
    public function certificate_list()
    {
        ...
        return view('land_use.certificate_list', compact("fiscal_year_list"));
    }
    ...
    public function money_receipt($sonod_no = null)
    {
        ...
        $response = $data->money_receipt_data($sonod_no, $union_id, $this->type);

        // dd($response);
        $bank = DB::table('bank')->where('sonod_type','=',96)->first();
        ...
        if (!empty($response)) {
            $pdf = PDF::loadView('land_use.money_receipt', ['data' => $response, 'bank'=> $bank, 'union' => $union_profile]);

            return $pdf->stream('land_use_money_receipt.pdf');
        }else{
    ...
    public function webApplly(WebLandUseInfoRequest $request)
    {
        ...
        $res = LandUse::webApplication($request);

        if($res){
            if(isset($request->web)){
                return response()->json(['success'=> 'আবেদনটি সম্পূর্ণ হয়েছে!', 'tracking' => $tracking, 'pin' => $request->pin, 'unionid' => $union_id, 'type' => $this->type, 'application' => 'land_use_application']);
            }else {
                Alert::alert()->html("<i>আবেদনটি সম্পূর্ণ হয়েছে!</i>","<p>আপনার পিন নং ".$request->pin." এবং ট্র্যাকিং নং ".$request->tracking."</p><a href='".route('land_use_preview', ['tracking' => $tracking])."' class='btn btn-info' target='_blank'><i class='icon-copy fa fa-eye' aria-hidden='true'></i> প্রিন্ট করুন</a><a href='applicant_list' class='btn btn-info ml-2'><i class='icon-copy fa fa-print' aria-hidden='true'></i>আবেদন তালিকা</a>",'success')->showConfirmButton($btnText = 'ঠিক আছে', $btnColor = '#3085d6')->persistent(false,true);
                return redirect()->back();
            }
        }else {
            if(isset($request->web)){
    ...
    public function register($from_date = null, $to_date = null){
        ...
        $pdf = PDF::loadHtml(view('land_use.register',  ['data' => $response, 'union' => $union_profile]), $config);

        return $pdf->stream("land_use_register.pdf");
    }
}

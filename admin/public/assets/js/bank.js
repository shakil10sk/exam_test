...
function bankList(){
    ...
    bank_table =  $('#bank_list_tbl').DataTable({
        scrollCollapse: true,
        autoWidth: false,
        responsive: true,
        serverSide: true,
        ...
        columns:[
...

            {
                ...
                render: function(data, type, row, meta){
                    ...
                    return (data.sonod_type == 1) ? "নাগরিক" : (data.sonod_type == 2) ? "মৃত্যু" : (data.sonod_type == 3) ? "অবিবাহিত" : (data.sonod_type == 4) ? "পুনঃবিবাহ" : (data.sonod_type == 5) ? "একইনাম" : (data.sonod_type == 6) ? "সনাতন" : (data.sonod_type == 7) ? "প্রত্যয়ন" : (data.sonod_type == 8) ? "নদীভাঙ্গন" : (data.sonod_type == 9) ? "চারিত্রিক" : (data.sonod_type == 10) ? "ভূমিহিন" : (data.sonod_type == 11) ? "বার্ষিক আয়" : (data.sonod_type == 12) ? "প্রতিবন্ধী" : (data.sonod_type == 13) ? "অনুমতি" : (data.sonod_type == 14) ? "ভোটার আইডি স্থানান্তর" : (data.sonod_type == 15) ? "অনাপত্তি" : (data.sonod_type == 17) ? "ওয়ারিশ" : (data.sonod_type == 18) ? "পারিবারিক" : (data.sonod_type == 19) ? "ট্রেড" : (data.sonod_type == 20) ? "বিবাহিত" : (data.sonod_type == 90) ? "প্রিমিসেস" : "N/A";
                }
            ...
            },
            {
                data:null,
                render: function(data, type, row, meta){
                    let htmlRender = " <button  onclick='bankEdit("+meta.row+")' class='btn btn-sm" +
                        " btn-warning'>এডিট</button> "+
                        " <button onclick='bankDelete("+meta.row+")'  class='btn btn-sm btn-danger" +
...
function bankEdit(row_index){

    let data = bank_table.row(row_index).data();
    console.log(data);

    $("#sonod_type").val(data.sonod_type);
    $("#bank_name").val(data.bank_name);
    $("#account_num").val(data.account_num);
    $("#bank_branch").val(data.bank_branch);
    $("#bank_branch_address").val(data.bank_branch_address);
    $("#pid").val(data.id);


    $("#bank_update_modal").modal('toggle');

}

function bankDelete(rowIndex){

    let data = bank_table.row(rowIndex).data();

...
// warish expire certificate list
var warish_expire_certi_table;

function warish_expire_certificate_list() {
    fiscal_year_id = $("#fiscal_year_id").val();
    from_date = $("#filter_from_date").val();
    to_date = $("#filter_to_date").val();

    warish_expire_certi_table = $('#warish_expire_certificate_table').DataTable({
        scrollCollapse: true,
        autoWidth: false,
        responsive: true,
        serverSide: true,
        ...
        ajax: {
            dataType: "JSON",
            type: "post",
            url: warish_expire_certificate_data_url,
            data: {
                fiscal_year_id: fiscal_year_id,
                from_date: from_date,
                to_date: to_date,
                _token: warish_expire_certificate_csrf
            },
        },
        columns: [{
            data: null,
            render: function () {
                return warish_expire_certi_table.page.info().start + warish_expire_certi_table.column(0).nodes().length;
            }
        },
            {
                data: null,
            ...
            },
            {
                ...
                render: function (data, type, row, meta) {
                    ...
                    return "<a  href='print_bn/" + data.sonod_no + "' target='_blank' ><p class='btn btn-sm btn-primary '>বাংলা</p></a>  <a  href='print_en/" + data.sonod_no + "' target='_blank' ><p class='btn btn-sm btn-success'>ইংরেজি</p></a> " + edit + regenerate;
                }
            },
        ],
        columnDefs: [{
...
function certificate_list_search() {
    ...
    warish_expire_certi_table.ajax.reload();
}
